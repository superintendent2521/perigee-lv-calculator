...
function missionRenderDetail() {
  ...
  const logHTML = m.log.length
    ? m.log.map((e, i) => `<div id="mlog-${id}-${i}" class="mcc-evt-row${_missionSelEvt===i?' sel':''}" style="position:relative;"><div onclick="missionSelectEvent('${id}',${i})" style="cursor:pointer">${_missionLogCardHTML(e)}</div><button class="act-btn" style="position:absolute;top:4px;right:4px;padding:1px 6px;font-size:10px;" onclick="event.stopPropagation();missionDeleteEvent('${id}',${i})" title="Delete event">✕</button></div>`).join('')
...
function _missionEventDetailHTML(m) {
  if (_missionSelEvt == null) return '';
  const e = m.log[_missionSelEvt];
  if (!e) return '';
  const id = m.missionId;
  let fields = '';
  if (e.type === 'LAUNCH') {
  ...
  return `<div class="mcc-section-header">EVENT DETAIL</div>
    <div class="mcc-panel-pad">
      ...
      <div style="font-family:var(--mono);font-size:9px;color:var(--text-dim);margin-top:6px;">// editing event parameters is a future enhancement</div>
      <div style="display:flex;gap:6px;margin-top:8px;">
        <button class="act-btn" onclick="missionDeleteEvent('${id}',${_missionSelEvt})">Delete Event</button>
        <button class="act-btn" onclick="missionSelectEvent('${id}',${_missionSelEvt})">Close</button>
      </div>
    </div>`;
}
