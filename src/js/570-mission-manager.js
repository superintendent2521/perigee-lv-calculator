...
function missionRenderDetail() {
  ...
  const logHTML = m.log.length
    ? m.log.map((e, i) => `<div id="mlog-${id}-${i}">${_missionLogCardHTML(e)}</div>`).join('')
...
function _missionBandViewHTML(m) {
  ...
  for (const lane of model.lanes) {
    ...
    if (lane.endIndex === null && !lane.expended && lane.points.length > 0) {
      ...
      lanesHTML += `<circle cx="${plusX}" cy="${plusY}" r="10" fill="var(--input)" stroke="${lane.color}" stroke-width="1.5" style="cursor:pointer" onclick="missionSetActiveVehicle('${id}','${lane.vehicleId}')"><title>Add event to ${lane.name}</title></circle>`;
