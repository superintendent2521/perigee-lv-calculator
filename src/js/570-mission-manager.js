...
function missionRenderDetail() {
  ...
  const logHTML = m.log.length
    ? m.log.map((e, i) => `<div id="mlog-${id}-${i}" class="mcc-evt-row${_missionSelEvt===i?' sel':''}" style="position:relative;"><div onclick="missionSelectEvent('${id}',${i})" style="cursor:pointer">${_missionLogCardHTML(e)}</div><button class="act-btn" style="position:absolute;top:4px;right:4px;padding:1px 6px;font-size:10px;" onclick="event.stopPropagation();missionDeleteEvent('${id}',${i})" title="Delete event">✕</button>${_missionSelEvt===i ? _missionEventDetailHTML(m, i) : ''}</div>`).join('')
...
function _missionEventDetailHTML(m, idx) {
  ...
  const isLatestBurn = e.type === 'BURN' && idx === m.log.length - 1 && PROG_ACTIVE_PROGRAM.vehicles[m.vehicleId];
  const editForm = isLatestBurn ? `
  ...
  const note = !isLatestBurn && e.type === 'BURN' ? `<div style="font-family:var(--mono);font-size:9px;color:var(--text-dim);margin-top:6px;">// inline editing currently supports the latest burn</div>` : '';
...
function missionExecBurn(id) {
  ...
  const _before = _missionSnapState(fv);
...
function missionApplyBurnEdit(id, idx) {
  ...
  const fv = PROG_ACTIVE_PROGRAM.vehicles[m.vehicleId]; if(!fv || !e.stateBefore) return;
  ...
  _missionRestoreState(fv, e.stateBefore);
  let r; try { r = _missionComputeBurn(fv, bt, pval); } catch(err){ return; }
  const ev = progMakeEvent('BURN', { vehicleId: m.vehicleId, stagingStageId: e.stageId||null, burnType: bt, dvTarget: r.dvTarget });
  const res = progDispatchEvent(PROG_ACTIVE_PROGRAM, ev);
  if (r.newOrbit && res.result!=='FAILED') { fv.orbitState = r.newOrbit; fv.status='ORBIT'; }
  e.burnType=bt; e.burnParam=pval; e.dvTarget=r.dvTarget; e.burnLabel=r.burnLabel;
  e.dv_actual=res.dv_actual||0; e.prop_consumed=res.prop_consumed||0; e.result=res.result;
  e.orbitAfter = fv.orbitState?{...fv.orbitState}:null;
  _missionSelEvt = null; missionRenderDetail();
}
...
function missionExecSeparate(id, sepIndex) {
  ...
  m.vehicleIds = m.vehicleIds.filter(v => v !== m.vehicleId);
  m.vehicleIds.push(res.lowerVehicleId, res.upperVehicleId);
  m.vehicleId = res.upperVehicleId;
  m.log.push({
    type: 'SEPARATE', result: 'SUCCESS',
    parentVehicleId: fv.vehicleId, lowerVehicleId: res.lowerVehicleId, upperVehicleId: res.upperVehicleId,
    parentName: fv.name, sepIndex: idx,
    lowerName: lower ? lower.name : '?', lowerStages: lower ? lower.stages.length : 0,
    upperName: upper ? upper.name : '?', upperStages: upper ? upper.stages.length : 0,
  });
  _missionAddEvt = null;
  _missionViewMode = 'nodemap';
  missionRenderDetail();
...
function missionExecDock(id, targetVehId) {
  ...
  m.vehicleIds = m.vehicleIds.filter(v => v !== m.vehicleId && v !== targetVehId);
  m.vehicleIds.push(res.vehicleId);
  m.vehicleId = res.vehicleId;
  m.log.push({
    type: 'DOCK', result: 'SUCCESS', aName, tName,
    aVehId: activeFV.vehicleId, tVehId: targetVehId, mergedVehicleId: res.vehicleId,
    mergedName: merged ? merged.name : '?', mergedStages: merged ? merged.stages.length : 0,
    warnings: ev.warnings || [],
  });
  _missionAddEvt = null;
  _missionViewMode = 'nodemap';
...
function missionExecExpendVehicle(id, vehId) {
  ...
  if (!fv) return;
  const name = fv.name;
  const ev = progMakeEvent('EXPEND', { vehicleId: vehId });
  const res = progDispatchEvent(PROG_ACTIVE_PROGRAM, ev);
  m.log.push({ type: 'EXPEND', vehicleLevel: true, vehicleId: vehId, vehicleName: name, result: res.result });
  if (m.vehicleId === vehId) {
    const next = _missionLiveVehicles(m).find(x => x.id !== vehId && x.fv.status !== 'EXPENDED');
    m.vehicleId = next ? next.id : vehId;
  }
  _missionAddEvt = null;
  _missionViewMode = 'nodemap';
  missionRenderDetail();
...
function _missionMultiVehicleHTML(m) {
  ...
  const rows = live.map(({ id: vid, fv }) => {
    ...
    return `<div style="display:flex;align-items:center;gap:8px;padding:5px 8px;border:1px solid ${isActive ? 'var(--accent)' : 'var(--border)'};margin-bottom:4px;background:${isActive ? 'var(--input)' : 'transparent'};">
      <button class="act-btn" style="padding:2px 8px;font-size:10px;${isActive ? 'background:var(--accent);color:#000;' : ''}" onclick="missionSetActiveVehicle('${id}','${vid}')">${isActive ? '● active' : 'select'}</button>
      <span style="font-family:var(--mono);font-size:11px;color:var(--text-bright)">${fv.name}</span>
      ...
      <button class="act-btn" style="padding:2px 8px;font-size:10px;margin-left:auto;" onclick="missionExecExpendVehicle('${id}','${vid}')"${expended ? ' disabled' : ''}>Expend</button>
    </div>`;
  }).join('');
...
function _missionNodeMapHTML(m) {
  ...
  const posOf = n => POS[n.id] || [n.cx || 0, n.cy || 0];
  ...
  for (const n of PROG_NM_NODES) {
    ...
    nodesHTML += `<g style="cursor:pointer" onclick="missionNodeClick('${id}','${n.id}')"><title>${n.label}${n.sub ? ' — ' + n.sub : ''}</title>`;
    if (isSurface && blob) {
      ...
      const r = n.r || 16;
      const stroke = isFrom ? 'var(--accent2)' : inPath ? 'var(--accent)' : 'var(--border-bright)';
  ...
  const svgHTML = `<svg viewBox="0 0 1100 520" preserveAspectRatio="xMidYMid meet" style="width:100%;height:auto;background:transparent;border:1px solid var(--border);">${blobsHTML}${edgesHTML}${nodesHTML}</svg>`;
